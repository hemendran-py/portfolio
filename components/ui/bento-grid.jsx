...
const BentoCard = ({
  ...
  <div
    ...
    <div     style={{
      ...
      backgroundSize: 'fill',  // Ensure the background image covers the entire card
    }}
>{background}</div>
    ...
    <div
  className={cn(
    "pointer-events-none absolute bottom-0 flex w-full translate-y-10 transform-gpu flex-row items-center p-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-90"
  )}>
  <Button
    variant="ghost"
    asChild
    size="xl"  // Tailwind size class for larger buttons
    className=" pointer-events-auto h-fit p-2  px-1 text-lg " // Adjusted size: taller button with larger text
  >
    <a href={href}>
      {cta}
